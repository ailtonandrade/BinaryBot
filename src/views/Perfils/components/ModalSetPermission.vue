        ...
        <div class="container">
          <div class="row area-content">
            ...
            <div class="col-lg-5 col-md-5 col-12 area-selector">
              ...
              <div v-for="(p) in selectedPerms" :key="'del-perm' + p.name">
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const filteredAblePerms = computed(() => {
      let names = selectedPerms.value?.map(s => s.name);
    ...
    const methods = reactive({
      ...
      setToRemove(event){
        selectedPerms.value = selectedPerms.value?.filter(p => p.name !== event?.name)
      },
      setIcon(event) {
      ...
      toAction() {
        configModal.value.description = selectedPerms.value.map(p => p.name).join(", ");
